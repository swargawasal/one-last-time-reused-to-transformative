...
def run_enhancement(input_path, output_path, config=None):
    ...
    enable_gemini = config.get("ENABLE_GEMINI_ENHANCE", "yes").lower() == "yes"
    ...
    # 2. Gemini AI Enhancement (Primary)
    if enable_gemini:
        ...
        if result == "SUCCESS":
            ...
            logger.warning("⚠️ Gemini failed, using FFmpeg MAX mode...")
            # Fallback to FFmpeg MAX (cpu_fast)
