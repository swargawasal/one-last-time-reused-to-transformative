...
def detect_watermark_with_gemini(image_path: str, original_video_path: str = None) -> Optional[Dict]:
    """
        ...
        try:
            ...
            if os.path.exists(meta_path):
                ...
                for k in keywords:
                    ...
                    # 1. Remove Special Characters FIRST (e.g. @Voompla -> Voompla)
                    # Remove ALL non-alphanumeric characters to be safe, or just leading?
                    # User said "remove the special charaters", usually implies leading/trailing or just symbols.
                    # Let's stick to removing leading symbols like @, # which are common in handles.
    ...
    try:
        ...
        if gemini_coords:
            ...
            if verified:
                ...
                try:
                    ...
                    if frame is not None:
                        opencv_watermark.learn(frame, gemini_coords, is_positive=True)
                except Exception as e:
                    logger.warning(f"⚠️ Failed to learn from Gemini: {e}")
            ...
            # FALLBACK: Try Adaptive OpenCV
            try:
                import opencv_watermark
                frame = cv2.imread(image_path)
                if frame is not None:
                    logger.info("   🧠 Attempting Adaptive OpenCV Fallback...")
                    cv_coords = opencv_watermark.detect(frame)
                    if cv_coords:
                        logger.info(f"   ✅ OpenCV Fallback Succeeded: {cv_coords}")
                        return cv_coords
            except Exception as e:
        ...
        logger.warning(f"⚠️ Gemini detection/verification skipped: {e}")
        ...
        try:
            ...
            frame = cv2.imread(image_path)
            if frame is not None:
                logger.info("   🧠 Attempting Adaptive OpenCV Fallback (Gemini Error)...")
                cv_coords = opencv_watermark.detect(frame)
                if cv_coords:
                    return cv_coords
        except Exception:
...
def process_video_with_watermark(input_path: str, output_path: str) -> bool:
    """
    ...
    try:
        # 0. Check for Manual Coordinates (Override)
        manual_coords_str = os.getenv("WATERMARK_MANUAL_COORDS", "")
        if manual_coords_str:
            try:
                mx, my, mw, mh = map(int, manual_coords_str.split(','))
                logger.info(f"🔧 Using Manual Watermark Coordinates: {mx},{my},{mw},{mh}")
                decision = {
                    "action": "replace",
                    "overlay": {
                        "x": mx, "y": my, "w": mw, "h": mh,
                ...
                # Skip detection
                analysis = {"logs": "Manual Override"}
            except ValueError:
                logger.error("❌ Invalid WATERMARK_MANUAL_COORDS format. Expected 'x,y,w,h'")
                pass
        
        # Only run detection if no manual override
        if "decision" not in locals():
            ...
            # 🔍 STEP 1: Gemini Detection (More Accurate)
            gemini_coords = detect_watermark_with_gemini(temp_frame)
            ...
            if gemini_coords:
                ...
                logger.info(f"🎯 Using VERIFIED Gemini coordinates for replacement.")
            else:
                # Fallback to OpenCV (already tried in detect_watermark_with_gemini fallback, but let's be sure)
                pass
        ...
        if enable_smart_crop and "crop_rect" in decision and decision["crop_rect"]:
            ...
            # None or Keep
            logger.info("   ✨ No watermark action needed.")
...
def detect_watermark(frame_path: str, template_path: Optional[str] = None) -> Optional[Dict]:
    ...
    try:
        ...
        result = opencv_watermark.detect(frame)
