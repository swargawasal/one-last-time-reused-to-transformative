...
TEXT_OVERLAY_TEXT = os.getenv("TEXT_OVERLAY_CONTENT", "🔥 VIRAL") # Updated key
...
def compile_with_transitions(input_video: Path, title: str) -> Path:
    ...
    final_output = os.path.join(OUTPUT_DIR, f"final_{safe_title}_{timestamp}.mp4")
    ...
    try:
        ...
        if width >= 1080 and height >= 1920:
            ...
            logger.info(f"📈 Resolution is below 1080x1920. Running enhancement...")
            scale = 2
        ...
        # 2.3 Watermark Detection + Replacement
        if WATERMARK_DETECTION:
            logger.info("🔍 Step 2.3: Watermark Detection + Replacement")
            coords = None
            
            # --- GEMINI DETECTION (Priority 1) ---
            if os.getenv("ENABLE_GEMINI_WATERMARK_DETECT", "yes") == "yes":
                frame_path = os.path.join(job_dir, "frame.jpg")
                if not os.path.exists(frame_path):
                    # We need a frame extraction function or use ffmpeg directly
                    cmd_frame = [FFMPEG_BIN, "-y", "-ss", "00:00:01", "-i", norm_video, "-vframes", "1", "-q:v", "2", frame_path]
                    _run_command(cmd_frame)
                
                if os.path.exists(frame_path):
                    from watermark_auto import detect_watermark_with_gemini
                    coords = detect_watermark_with_gemini(frame_path, str(input_path))
            ...
            if not coords:
                ...
                if os.path.exists(frame_path):
                    coords = detect_watermark(frame_path)
            
            if coords:
                logger.info(f"🎯 Actioning Watermark at {coords}")
                watermarked_video = os.path.join(job_dir, "watermarked.mp4")
                wm_success = False
                
                # Check for REMOVAL first (User Preference)
                if WATERMARK_REMOVE:
                    logger.info("   🧹 Removing watermark (Delogo)...")
                    try:
                        # Ensure even dimensions for delogo
                        w = coords['w'] - (coords['w'] % 2)
                        h = coords['h'] - (coords['h'] % 2)
                        
                        delogo_filter = f"delogo=x={coords['x']}:y={coords['y']}:w={w}:h={h}:show=0"
                        ...
                        if _run_command(cmd_delogo, check=True):
                            wm_success = True
                    except Exception as e:
                        logger.error(f"❌ Delogo failed: {e}")

                # If removal didn't happen or wasn't requested, try REPLACE
                if not wm_success and (os.getenv("WATERMARK_REPLACE_PATH", "").strip() or os.getenv("MY_WATERMARK_TEXT", "").strip()):
                    if os.getenv("WATERMARK_REPLACE_PATH", "").strip() and os.path.exists(os.getenv("WATERMARK_REPLACE_PATH")):
                        wm_path = os.getenv("WATERMARK_REPLACE_PATH")
                        wm_success = apply_my_watermark(
                            norm_video,
                            watermarked_video,
                            wm_path,
                            coords,
                            opacity=MY_WATERMARK_OPACITY
                        )
                    elif os.getenv("MY_WATERMARK_TEXT", "").strip():
                        wm_text = os.getenv("MY_WATERMARK_TEXT")
                        wm_success = apply_text_watermark(
                            norm_video,
                            watermarked_video,
                            wm_text,
                            coords,
                ...
                if wm_success and os.path.exists(watermarked_video):
                    norm_video = watermarked_video
                    logger.info("   ✅ Watermark processed successfully.")
                    
                    # CRITICAL: Set wm_context so main.py knows we detected something!
                    wm_context = coords

        # 2.4 Smart Crop Application
        if smart_crop_box:
            logger.info("✨ Step 2.4: Applying Smart Crop")
            cropped_video = os.path.join(job_dir, "smart_cropped.mp4")
            try:
                crop_filter = f"crop={smart_crop_box['w']}:{smart_crop_box['h']}:{smart_crop_box['x']}:{smart_crop_box['y']}"
                ...
                _run_command(cmd_crop, check=True)
                if os.path.exists(cropped_video):
                    norm_video = cropped_video
            except Exception as e:
                logger.warning(f"⚠️ Smart crop failed: {e}")
        ...
        if voiceover_path and os.path.exists(voiceover_path):
            try:
                ...
                cmd_mix = [
                    ...
                    "-filter_complex", "[1:a]volume=2.0[vo];[0:a]volume=0.5[bg];[bg][vo]amix=inputs=2:duration=first",
                    "-c:v", "copy",
                    "-c:a", "aac",
                    mixed_vo_video
                ]
                _run_command(cmd_mix, check=True)
                if os.path.exists(mixed_vo_video):
